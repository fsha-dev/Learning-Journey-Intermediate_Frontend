...
async function generator(config) {
  ...
  if (config.virtualRouteConfig) {
    getRouteNodesResult = await getRouteNodes.getRouteNodes(config);
  } else {
    getRouteNodesResult = await getRouteNodes$1.getRouteNodes(config);
  }
...
export const Route = createRootRoute({
  component: () => (
    <React.Fragment>
      <div>Hello "${rootPathId.rootPathId}"!</div>
      <Outlet />
    </React.Fragment>
  ),
})
...
`;
      ...
      fs__namespace.writeFileSync(
        node.fullPath,
        await prettier__namespace.format(replaced, prettierOptions)
      );
    }
  };
